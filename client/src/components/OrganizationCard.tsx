...
import {Organization, SummaryData} from '../types'
...
export default class OrganizationCard extends Component<Props> {
  ...
  formatDate = (date: number) => {
    ...
    if (date) {
      ...
      if (!filingDate) return dateString
  ...
  percentageFormatter = (number: number|string, key: string) =>{
    const intNumber = typeof number === 'number' ? number : parseInt(number, 10)
    const base = this.props.summaryData[key]
    if (!base) return null
    let percentage: number|string = ((intNumber - base) / base)
                ...
                <dl className={hideOnClick}>
                  ...
                  <dd className="card-text">
                    <a target="_blank" href={'http://maps.google.com/?q='+this.formatAddress(organization)}>
                      {startCase(toLower(organization.city)) + ", " + organization.state}
                    </a>
                  </dd>
                </dl>
